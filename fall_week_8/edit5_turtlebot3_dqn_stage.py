...
class ReinforceAgent():
    ...
    def appendMemory(self, state, action, reward, next_state, done):
        # (edit5) while appending memory, also need to calculate error and see how much
        # (edit5) prioritization will be given
        td_error =  reward + self.discount_factor * np.max(self.target_model.predict(next_state))\
                    - self.model.predict(state)
        p = np.abs(td_error + 0.000000001) ** self.alpha
        self.memory.append((state, action, reward, next_state, done))
        self.priority.append(p)

    # (edit5) return experience replay according to prioritization
    # (edit5) also needs to return a value for importance sampling
    ...
    def trainModel(self, target=False):
        ...
        for i in range(self.batch_size):
            ...
            # (edit5) calculate weight for importance sampling
            ...
            # (edit5) create a batch for importance as well
            # (edit5) to be accounted for when doing model.fit
            # (edit5) meaning that parameters will be updated proportionally to importance
        ...
        # (edit5) incorporate sample_weight for importance sampling :)
    ...
    for e in range(agent.load_episode + 1, EPISODES):
        ...
        for t in range(agent.episode_step):
            ...
            if done:
                ...
                agent.updateTargetModel()   # (edit2) copy to line to below
                                            # (edit2) to update target_model after a completed episode as well
                scores.append(score)
                episodes.append(e)
            ...
            if global_step % agent.target_update == 0:
                ...
                # (edit2) added agent.updateTargetModel()
