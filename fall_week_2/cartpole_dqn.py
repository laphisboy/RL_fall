...
def train(q, q_target, memory, optimizer):
    for i in range(10):
        ...
        max_q_prime = q_target(s_prime).max(1)[0].unsqueeze(1)              # yeah choose the max possible value

        target = r + gamma * max_q_prime * done_mask                        # target y .. or j
...
def main():
    ...
    for n_epi in range(10000):
        ...
        if memory.size()>2000:                                      # hmm why train when episode terminates?
            train(q, q_target, memory, optimizer)                   # shouldn't it be after each episode?
