...
@tf.function
def compute_gradient(states, critic, actor, polyak, actions, mus,
                     dones, rewards, gamma, batch_size, action_size, c, delta):
    with tf.GradientTape(persistent=True) as tape:
        tape.watch(states)

        q = critic(states)
        pi = actor(states)
        pi_avg = polyak(states)
        ...
        # v = tf.reduce_sum(q * pi, axis=-1)
        v = q * pi
        rho = pi / (tf.reshape(mus, [batch_size, action_size]) + 1e-6)
        rho_a = tf.gather_nd(rho, a_index)
        rho_bar = tf.minimum(1.0, rho_a)
        # print(a_index)
        # print(pi)
        # print(tf.reshape(mus, [batch_size, action_size]))
        # print(rho)
        # print(rho_a)
        # print(rho_bar)
        # print(v)
        ...
        for i in reversed(range(len(rewards))):
            # print(q_ret)
            # print(rewards[i])
            q_ret = tf.reduce_sum(rewards[i] + gamma * q_ret, axis=-1)
            q_rets.append(q_ret)
            q_ret = (rho_bar[i] * (q_ret - q_a[i])) + v[i]

        q_rets.reverse()
        # q_rets = tf.reduce_sum(q_rets, axis=1)
        # print(q_rets)
        # print(len(q_rets))
        q_ret1 = tf.convert_to_tensor(q_rets, dtype=tf.float32)
        q_ret2 = tf.expand_dims(q_ret1, axis=1)
        q_ret = tf.reshape(q_ret2, [batch_size, ])

        # print(rho_bar)
        # print(tf.math.log(pi_a + 1e-6))
        # print(q_ret - tf.reduce_sum(v,axis=-1))
        loss_f = - rho_bar * tf.math.log(pi_a + 1e-6) * (q_ret - tf.reduce_sum(v, axis=-1))
        # loss_bc = - tf.maximum((1 - c / rho), 0.0) * pi * tf.math.log(pi) * (
        #        q - tf.stack([v, v, v], axis=1))  # note that tf.____ functions might need to be
        loss_bc = - tf.maximum((1 - c / rho), 0.0) * pi * tf.math.log(pi) * (
        ...
        # print(g)
        # print(k)
        loss_pi = tf.maximum(0.0, (tf.reduce_sum(k * tf.stack([g, g, g], axis=1), axis=-1) - delta) / (
                    tf.reduce_sum(k * k, axis=-1) + 1e-6))
        loss_v = loss_q
    grad_pi = tape.gradient(loss_pi, actor.trainable_variables)
    grad_v = tape.gradient(loss_v, critic.trainable_variables)
    # print(grad_pi)
    # print(grad_v)
    # return zip(grad_pi, actor.trainable_variables), zip(grad_v, critic.trainable_variables)
    return grad_pi, grad_v
...
class ReinforceAgent():  # ACER
    def __init__(self, state_size, action_size):
        ...
        if self.load_model:
            self.actor.set_weights(
                tf.keras.models.load_model(self.dirPath + 'actor_' + str(self.load_episode) + ".h5").get_weights())
            self.critic.set_weights(
                tf.keras.models.load_model(self.dirPath + 'critic_' + str(self.load_episode) + ".h5").get_weights())
            self.polyak.set_weights(
                tf.keras.models.load_model(self.dirPath + 'polyak_' + str(self.load_episode) + ".h5").get_weights())
    ...
    def trainModel(self, on_policy=False, target=False):
        ...
        grad_pi, grad_v = compute_gradient(states, self.critic, self.actor, self.polyak,
                                           actions, mus, dones, rewards, self.gamma,
                                           self.batch_size, self.action_size, self.c, self.delta)

        # return zip(grad_pi, actor.trainable_variables), zip(grad_v, critic.trainable_variables)
        trainer_pi.apply_gradients(zip(grad_pi, self.actor.trainable_variables))
        trainer_v.apply_gradients(zip(grad_v, self.critic.trainable_variables))
...
if __name__ == '__main__':
    ...
    for e in range(agent.load_episode + 1, EPISODES):
        ...
        for t in range(agent.episode_step):
            ...
            if e % 10 == 0:
                agent.actor.save(agent.dirPath + 'actor_' + str(e) + '.h5')
                agent.critic.save(agent.dirPath + 'critic_' + str(e) + '.h5')
                agent.polyak.save(agent.dirPath + 'polyak_' + str(e) + '.h5')
