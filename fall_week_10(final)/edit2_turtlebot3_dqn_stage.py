...
class ReinforceAgent():	# ACER
	...
	input = tf.keras.layers.Input(shape=(self.state_size,))
        dense1 = tf.keras.layers.Dense(256, input_shape=(self.state_size,), activation='relu', kernel_initializer='lecun_uniform', name='dense1')(input)
        #dense2 = Dense(64, activation='relu', kernel_initializer='lecun_uniform'))
	pi = tf.keras.layers.Dense(self.action_size, kernel_initializer='lecun_uniform', activation='softmax', name='pi')(dense1)
	# or try...
...
#	return
	...
	print(loss_f)
	#print(rho_bar * tf.log(pi_a + 1e-6))
	print(q_ret)
	print(v)

	print(q_ret - v)	
	#loss_f = tf.reduce_mean(loss_f)
	loss_bc = - tf.maximum((1-self.c/rho),0.0) * pi * tf.log(pi) * (q - tf.stack([v, v, v], axis=1))	# note that tf.____ functions might need to be
	loss_bc = tf.reduce_sum(loss_bc, axis=1)
									# tf.math.____
	#loss_bc = - tf.maximum((1-self.c/rho),0.0) * pi * tf.log(pi) * (tf.reduce_sum(q,axis=1) - v)	# note that tf.____ functions might need to be
	...
	grad_pi = tf.gradients(grad_pi, self.actor.trainable_variables)
	grad_v = tf.gradients(loss_q, self.critic.trainable_variables)

	trainer_pi = tf.train.AdamOptimizer(learning_rate=self.learning_rate)
	trainer_v = tf.train.AdamOptimizer(learning_rate=self.learning_rate)
